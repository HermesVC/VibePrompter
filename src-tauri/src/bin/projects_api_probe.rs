//! Agent scenario: scan ProjectsAPI.php for bugs and apply fix via tools.

use app_lib::app::probe::{
    build_probe_state, probe_apply_patch_smoke, probe_projects_api_bugfix,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut report = probe_projects_api_bugfix(&state).await?;
    ...
    if report.had_bug_before && report.has_bug_after {
        ...
        anyhow::bail!("FAIL: bug still in file after agent run");
    }

    println!("PASS: agent scenario completed");
    Ok(())
}
